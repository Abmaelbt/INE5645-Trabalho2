...
void upload_file(int socket, const char *file_path, const char *remote_path) {
    ...
    while ((bytes_read = fread(buffer, 1, TRANSFER_RATE, file)) > 0) {
        ...
        usleep(1000000 * bytes_read / TRANSFER_RATE); // Controle de taxa
    }
    ...
    if (total_sent == file_size) {
        ...
        if (recv(socket, confirmation, sizeof(confirmation), 0) > 0 && strcmp(confirmation, "SUCCESS") == 0) {
            printf("Envio de '%s' concluído. Excluindo arquivo local...\n", file_path);
            delete_file(file_path); // Remover arquivo enviado
        } else {
